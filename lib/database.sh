cat > lib/database.sh << 'FROSTY_EOF'
#!/usr/bin/env bash
# ============================================================
# Frosty.exe - Phase 2 Step 4: MariaDB & Redis
# ============================================================
...
install_database() {
    ...
    if command -v mariadb >/dev/null 2>&1 || command -v mysql >/dev/null 2>&1; then
        _frosty_ok "MariaDB/MySQL client already present"
    fi
    ...
    if [[ -d /run/systemd/system ]]; then
        ...
        _frosty_warn "No systemd — starting mariadb manually via service command"
        service mariadb start >/dev/null 2>&1 || mysqld_safe --skip-grant-tables=0 & >/dev/null 2>&1
        sleep 3
    fi

    # Verify MariaDB is actually accepting connections
    local mysql_ready=0
